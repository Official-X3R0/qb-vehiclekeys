...
function LockPickDoorEvent(vehicle)
    loadAnimDict("veh@break_in@0h@p_m_one@")
    TaskPlayAnim(PlayerPedId(), "veh@break_in@0h@p_m_one@", "low_force_entry_ds", 3.0, 3.0, -1, 16, 0, 0, 0, 0)
    exports['skillchecks']:startLockpickingGame(20000, 6, 5, function(success)             -- Duration, Number of locks, Number of levels
    -- exports['skillchecks']:startAlphabetGame(20000, 5, function(success)                -- Duraction, Number of keys to press
    -- exports['skillchecks']:startFloodGame(20000, 6, 6, function(success)                -- Duraction, Move count, Grid size
    -- exports['skillchecks']:startWordsGame(20000, 6, function(success)                   -- Duraction, Required correct choices
    -- exports['skillchecks']:startUntangleGame(20000, 6, function(success)                -- Duraction, Number of points
    -- exports['skillchecks']:startDirectionGame(20000, 3, 3, 6, function(success)         -- Duraction, Required correct choices, Min. grid size, Max. grid size
    -- exports['skillchecks']:startSameGame(20000, 3, 3, function(success)                 -- Duraction, Grid size X, Grid size Y
    -- exports['skillchecks']:startFlipGame(20000, 5, function(success)                    -- Duraction, Grid size
        if success then
            LockpickFinishCallback(success)
        else
            exports['ps-dispatch']:CarJacking(vehicle)
            TriggerServerEvent('hud:server:GainStress', math.random(1,4))
            TriggerEvent("QBCore:Notify", "Failed lockpicking attempt.", "error")
        end
    end)
end
